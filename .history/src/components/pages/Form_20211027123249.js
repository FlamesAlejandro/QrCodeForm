...
import { DataCamioneros } from '../data/DataCamioneros';
import { DataEmpresaCamion } from '../data/DataEmpresaCamion';
import { DataCamion } from '../data/Datacamion';
import { DataEmpresaDestino } from '../data/DataEmpresaDestino';
...
export const Form = () => {
    ...
    const  handleChofer = ( { target }) => {
        ...
        if (DataCamioneros.find( chofer => chofer.rut === value)){

            const newdata = DataCamioneros.filter( chofer => chofer.rut === value);
    ...
    const  handleEmpresaCamion = ( { target }) => {
        const { name , value } = target;
        console.log('este es el name'+name, 'este es el value'+value);

        if (DataEmpresaCamion.find( empresaC => empresaC.rutempresa === value)){

            const newdata = DataEmpresaCamion.filter( empresaC => empresaC.rutempresa === value);
    ...
    const  handlePatente = ( { target }) => {
        const { name , value } = target;

        if (DataCamion.find( camion => camion.patente === value)){

            const newdata = DataCamion.filter( camion => camion.patente === value);
    ...
    const  handleEmpresaDestino = ( { target }) => {
        const { name , value } = target;
  
        if (DataEmpresaDestino.find( EmpresaDes => EmpresaDes.rutdestino === value)){

            const newdata = DataEmpresaDestino.filter( EmpresaDes => EmpresaDes.rutdestino === value);
        
            setFormulario({
                ...formulario,
                rutChofer: newdata[0].rutdestino,
                nombreChofer: newdata[0].nombredestino,
                correoChofer: newdata[0].ciudad,
                telefonoChofer: newdata[0].potrero,
            });
            console.log(newdata);
            ...
            <Container component="main">
                ...
                <form onSubmit={onSubmit} sx={{ mt: 3, width: '100%', }}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="rutEmpresa" name="rutEmpresa" value={rutEmpresa} onChange={handleChange} onBlur={handleEmpresaCamion} />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="patenteCamion" name="patenteCamion" value={patenteCamion} onChange={handleChange} onBlur={handlePatente}  />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="rutEmpresaDestino" name="rutEmpresaDestino" value={rutEmpresaDestino} onChange={handleChange} onBlur={handleEmpresaDestino}  />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="variedadCarga" name="variedadCarga" value={variedadCarga} onChange={handleChange}  />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="TaraCamion" name="TaraCamion" value={TaraCamion} onChange={handleChange}  />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="codigoMOP" name="codigoMOP" value={codigoMOP} onChange={handleChange}  />
                        </Grid>
                    </Grid>
                    <Button
                        type="submit"
