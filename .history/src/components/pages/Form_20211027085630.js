...
export const Form = () => {
    ...
    const  handleChofer = ( { target }) => {
        ...
        const { id, rut, nombre, correo, telefono } = camioneros.filter( chofer => chofer.rut === value);

        // const { id, rut, nombre, correo, telefono } = newdata;
        console.log(nombre);
        console.log(formulario);

        if({ id, rut, nombre, correo, telefono }){
            setFormulario({
                ...formulario,
                rutChofer: rut,
                nombreChofer: nombre,
            ...
            console.log({ id, rut, nombre, correo, telefono });
            console.log(formulario);
        }        
                ...
                <form onSubmit={generarQrCode} sx={{ mt: 3, width: '100%', }}>
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="NombreChofer"  />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <TextField fullWidth type="text" placeholder="RutChofer" name="RutChofer" value={formulario.rutChofer} onChange={handleChofer} />
                        </Grid>
                        <Grid item xs={12} sm={6}>
